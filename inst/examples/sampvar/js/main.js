...
var showAnimation = function(obj) {
    ...
    var cloneSampleNodes = function(info) {
        var dataPointsSel = getSVGMappings("data-points", "grob", "selector")[0];
        var iter = info.vecNum[0] - 1;
        var sampNodes = $(dataPointsSel + " use")
            .filter(function(i) {
                return _.contains(sampInds[iter], i + 1);
            })
            .clone()
            .appendTo(dataPointsSel)
            .get();
    ...
    var dropSample = function(info) {
        var dataPointsSel = getSVGMappings("data-points", "grob", "selector")[0];
        var iter = info.vecNum[0] - 1;
        d3.selectAll(dataPointsSel + " .cloned")
    ...
    var removeSample = function(info) {
        var dataPointsSel = getSVGMappings("data-points", "grob", "selector")[0];
        d3.selectAll(dataPointsSel + " .cloned").remove();
    };
    ...
    var addStatLine = function(info) {
        ...
        var sampleXAxisSel = getSVGMappings("sample-xaxis", "grob",
                                            "selector")[0];
        $(sampleXAxisSel).after(newGhost);
